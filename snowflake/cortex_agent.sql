...
CREATE OR REPLACE AGENT APP.PDM_AGENT
  ...
  FROM SPECIFICATION $$
  {
    ...
    "instructions": {
      ...
      "response": "Be concise and actionable. Structure recommendations as:\n1. Key signals from telemetry and predictions (cite specific sensor values using the Trends tab names — e.g. VIBRATION_RMS not vibration for pumps, BEARING_TEMP not temperature)\n2. Relevant manual excerpts with source citations (reference the specific equipment model manual)\n3. Clear action list with parts needed (include co-replacement suggestions — e.g. 'Part X typically wears at the same time as Part Y, suggest replacing both')\n4. Safety cautions if applicable\n\nUse technical language appropriate for field engineers. Include specific sensor values and thresholds when available. IMPORTANT: Always use the sensor names exactly as they appear in the Trends tab so users can cross-reference. When suggesting parts, always consider what else typically needs replacement at the same time based on maintenance history."
    },
    "tools": [
      {
        "tool_spec": {
